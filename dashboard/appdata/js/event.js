...
$("#enable-detection-all").on('click', function (event) {
 ...
 function getZip(row) {
	 ...
	 createZipXhr = $.ajax({
		 url: "/lib/prismaCore/1/event/createzip/" + data[0],
		 async: true,
		 global: false,
		 method: 'POST',
	 ...
	 table2 = $('#DetectionList').DataTable({
		 ...
		 "drawCallback": function (oSettings) {
			 if (zipDownload) {
				 ...
				 window.location.href = "/lib/prismaCore/1/event/download/" + zipName.split(".")[0];
				 zipName = null;
				 zipDownload = false;
			 }
		 },
		 ...
		 bServerSide: true,
		 bStateSave: true,
		 sAjaxSource: '/lib/prismaCore/1/event/datatable/filelist',
		 "paging": true,
		 "bLengthChange": false,
		 "ordering": true,
		 "info": true,
