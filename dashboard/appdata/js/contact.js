...
function setLastEditId() {
    lastEditId = core_person.idPerson;
}
...
function saveObj() {
    ...
    contactLogic.save(core_person, setIndexToShow, setLastEditId, f, reloadAllDatatable);
}
...
function setIndexToShow() {
    indexToShow = core_person.idPerson;
}
...
function initFilters() {
    $(".filter-text").each(function (index) {
        $(this).select2({
            ...
            multiple: true,
            ajax: {
                url: '/lib/prismaCore/v1/person/autocomplete/' + $(this).attr('id').replace('F_', ''),
                dataType: 'json'
            },
