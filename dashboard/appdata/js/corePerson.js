...
$(setCorePersonVisibility());
var prcoreperson = new CorePersonModel('1');
var lastEditId = '';
var indexToShow = null;
$(function(){
	disableForm(prcoreperson);

});

function setLastEditId(){
	lastEditId = prcoreperson.id;
}

function editObj(id){
	disableForm(prcoreperson,true);
	corepersonLogic.get(prcoreperson,id);
	$('td').removeClass('lastEditedRow');
	lastEditId = '';
}

function allowEditObj(){
	enableForm(prcoreperson,false);
}

function saveObj(){
	var f = function(){disableForm(prcoreperson);}
	corepersonLogic.save(prcoreperson,setIndexToShow,setLastEditId, f, reloadAllDatatable);
}

function removeObj(){
	var f = function(){disableForm(prcoreperson);}
	corepersonLogic.remove(prcoreperson,prcoreperson.id, safeDelete, f, reloadAllDatatable);
}

function newObj(){
	newForm(prcoreperson);
	corepersonLogic.get(prcoreperson, null);
	$('td').removeClass('lastEditedRow');
	lastEditId = '';
}
...
function setIndexToShow(){
	indexToShow = prcoreperson.id;
}

$(document).ready(function () {
	table = $('#CorePersonList').DataTable({
		"oLanguage": {
		...
		"fnServerParams": function (aoData) {
			...
			if ($("." +$.md5('is_station_referer')).is(":visible"))
				aoData.push({"name": "is_station_referer", "value": $('#F_is_station_referer').val()});
			if ($("." +$.md5('is_administrator')).is(":visible"))
				aoData.push({"name": "is_administrator", "value": $('#F_is_administrator').val()});
		},
		...
		bServerSide: true,
		bStateSave: true,
		sAjaxSource: '/lib/prismaCore/1/coreperson/datatable/list'
	});
});
...
function initFilters() {
	$(".filter-text").each(function (index) {
		$(this).select2({
			...
			multiple: true,
			ajax: {
				url: '/lib/prismaCore/1/coreperson/autocomplete/' + $(this).attr('id').replace('F_',''),
				dataType: 'json'
			},
	...
	$(".foreign_key").each(function (index) {
		$(this).select2({
			...
			multiple: false,
			ajax: {
				url: '/lib/prismaCore/1/coreperson/foreignkey/' + $(this).attr('id').replace('F_',''),
				dataType: 'json'
			},
