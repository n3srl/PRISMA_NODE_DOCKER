...
$('table:not(.noclick) tbody ').on('click', 'tr', function (e) {
    ...
    if ($(this).hasClass('selected')) {
        ...
        } else if ($(this).closest("table").attr("id") == 'CorePersonList') {
            //editObj(d[d.length - 1]);
            editPerson(d[d.length - 1]);
        } else {
            editObj(d[d.length - 1]);
        }
...
function hideAllForm(formName = "") {
     ...
     * PERSON
     */
    $(formName + "#savebtnPerson").hide();
    $(formName + "#deletebtnPerson").hide();
    $(formName + "#cleanbtnPerson").hide();
    $(formName + "#modifybtnPerson").hide();
    $(formName + "#undobtnPerson").hide();
}
function newForm(objClass, editButton = false, isPerson = false) {
    var formSelector = "";
    ...
    if (isPerson) {
        $(formSelector + "#savebtnPerson").show();
    } else {
        $(formSelector + "#savebtn").show();
    }
    $(formSelector + "#deletebtn").hide();
    $(formSelector + "#deletebtnPerson").hide();
    if (editButton) {
        if (isPerson) {
            $(formSelector + "#cleanbtnPerson").show();
            $(formSelector + "#modifybtnPerson").hide();
        } else {
...
function enableForm(objClass, editButton = false, isPerson = false) {
    var formSelector = "";
    if (objClass.form_name != undefined) {
        formSelector = "[name='" + objClass.form_name + "'] ";
    }
    ...
    if (isPerson) {
        $(formSelector + "#savebtnPerson").show();
        $(formSelector + "#deletebtnPerson").show();
        $(formSelector + "#cleanbtnPerson").show();
        $(formSelector + "#undobtnPerson").show();
    } else {
        $(formSelector + "#savebtn").show();
    ...
    if (editButton) {
        if (isPerson) {
            $(formSelector + "#modifybtnPerson").show();
        } else {
...
function disableForm(objClass, editButton = false, isPerson = false) {
    var formSelector = "";
    if (objClass.form_name != undefined) {
        formSelector = "[name='" + objClass.form_name + "'] ";
    }
    ...
    if(isPerson)
    {
        $(formSelector + "#savebtnPerson").hide();
        $(formSelector + "#cleanbtnPerson").hide();
        $(formSelector + "#deletebtnPerson").hide();
    } else{
...
function getGeneratedCode(objClass, region_id, ...callBack) {
    objClass.getCode(region_id, ...callBack);

}

function getGeneratedHostname(objClass, region_id, ...callBack) {
    objClass.getHostname(region_id, ...callBack);

}

function getPersonId(objClass, prnode, id, ...callback) {
    objClass.getUserId(id, prnode, ...callback);
}

function updateActiveStatus(objClass, ...callback) {
    objClass.updateActive(...callback);
}
...
function saveClass(objClass, ...callBack) {
    ...
    setTimeout(() => {
        ...
        let safe = false;
        if (objClass.id > 0 || objClass.idPerson > 0) {
            safe = true;
        }
        if (safe) {
