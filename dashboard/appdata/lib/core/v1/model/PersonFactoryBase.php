...
class PersonFactoryBase {
    ...
    public static function Save($object) {
        ...
        $query = "INSERT INTO core_person 
        (id,oid,username,password,email,title,first_name,middle_name,last_name,suffix,
        company,job_title,web_page_address,im_address,phone,address,postcode,
        number,city,province,country,timezone,created_by,modified_by,assigned,is_station_referer,is_administrator,
        create_date,valid_from,valid_to,erased,last_update) 
        VALUES ( null,$ob->oid,$ob->username,$ob->password,$ob->email,$ob->title,$ob->first_name,
        $ob->middle_name,$ob->last_name,$ob->suffix,$ob->company,$ob->job_title,
        $ob->web_page_address,$ob->im_address,$ob->phone,$ob->address,$ob->postcode,$ob->number,
        $ob->city,$ob->province,$ob->country,$ob->timezone,$ob->created_by,$ob->modified_by,$ob->assigned,$ob->is_station_referer,$ob->is_administrator,
        $ob->create_date,$ob->valid_from,$ob->valid_to,0,now())";
    ...
    public static function Update($object) {
        ...
        $query = "UPDATE core_person SET oid =$object->oid,
        username =$object->username,password =$object->password,
        title =$object->title,first_name =$object->first_name,
        middle_name =$object->middle_name,last_name =$object->last_name,
        suffix =$object->suffix,company =$object->company,
        job_title =$object->job_title,email =$object->email,web_page_address =$object->web_page_address,
        im_address =$object->im_address,phone =$object->phone,address =$object->address,
        postcode =$object->postcode,number =$object->number,city =$object->city,
        province =$object->province,country =$object->country,timezone =$object->timezone,
        modified_by = $object->modified_by,valid_from = $object->valid_from,
        valid_to = $object->valid_to,erased = $object->erased,
        last_update=now() WHERE id=" . $object->id;
    ...
    public static function Get($id) {
        ...
        if ($res === false || mysqli_num_rows($res) <= 0)
            return false;
        $object = mysqli_fetch_object($res, "Person");
