...
class GroupApiLogic {
    ...
    public static function Update($request) {
        ...
        try {
            ...
            $ob = GroupLogic::Get($ob->id);

            CoreLogic::getFromArray($ob, $request->get("data"));

            CoreLogic::beginTransaction();
            $res = GroupLogic::Update($ob);
            CoreLogic::commitTransaction();
        } catch (ApiException $a) {
            CoreLogic::rollbackTransaction();
            return CoreLogic::GenerateErrorResponse($a->message);
    ...
    public static function GetListFilterAjax($columnName) {
        try {
            ...
            $results = array();
            $data = new stdClass();
            $codes = GroupFactory::GetListFilter($columnName, $_GET['term']);
    ...
    public static function GetListFKAjax($columnName) {
        try {
            ...
            $results = array();
            $data = new stdClass();
            $foreignKey = end(GroupFactory::GetForeignKeyParams($columnName));
            $codes = GroupFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME, $foreignKey->REFERENCED_COLUMN_NAME, $_GET['term']);
            $data = new stdClass();
            foreach ($codes as $code) {
                $obj = new stdClass();
     ...
     public static function GetListDropdown() {
        try {
            $Person = CoreLogic::VerifyPerson();
            $ob = GroupLogic::GetListSearch($_GET['term']);
            $data = new stdClass();
            foreach ($ob as $code) {
                $obj = new stdClass();
                $obj->id = $code->id;
                $obj->text = $code->name;
                if($code->type == Group::$AG){
                    $obj->disabled = true;
                }
                $results[] = $obj;
            }
    ...
    public static function GetListDatatable() {
        ...
        $aColumns = array('`oid`', '`name`', '`type`', '`id`');
        $aColumnsName = array('oid', 'name', 'type', 'id');
        $sIndexColumn = 'id';
        $sTable = 'core_group';
        $gaSql['link'] = $db_conn;
        ;
