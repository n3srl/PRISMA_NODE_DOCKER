...
class CameraApiLogic
{
	...
	public static function Update($request){
		...
		try {
			...
			$ob = CameraLogic::Get($ob->id);

			CoreLogic::getFromArray($ob, $request->get("data"));

			CoreLogic::beginTransaction();
			$res = CameraLogic::Update($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function GetListFilterAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
		$codes = CameraFactory::GetListFilter($columnName,$_GET['term']);
	...
	public static function GetListFKAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
			$foreignKey = end(CameraFactory::GetForeignKeyParams($columnName));
			$codes = CameraFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME,$foreignKey->REFERENCED_COLUMN_NAME,$_GET['term']);
			$data = new stdClass();
			foreach ($codes as $code){ 
				$obj = new stdClass(); 
	...
	public static function GetListDatatable() {
		...
		$aColumns = array('`oid`','`node_id`','`code`','`config_file`','`mask_file`','`model`','`id`');
		$aColumnsName = array('oid','node_id','code','config_file','mask_file','model','id');
		$sIndexColumn = 'id';
		$sTable = 'pr_camera';
		$gaSql['link'] = $db_conn;;
		/*         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
		...
		while ($aRow = mysqli_fetch_array($rResult)) {
			...
			for ($i = 0; $i < count($aColumnsName); $i++) {
				if ($aColumnsName[$i] == 'id') {
					$row[] = $id;
				} else if ($aColumnsName[$i] == 'node_id') {
                                    $Node = NodeLogic::Get($aRow[$aColumnsName[$i]]);
					$row[] = "$Node->CName";
