...
class StationApiLogic
{
	...
	public static function Update($request){
		...
		try {
			...
			$ob = StationLogic::Get($ob->id);

			CoreLogic::getFromArray($ob, $request->get("data"));

			CoreLogic::beginTransaction();
			$res = StationLogic::Update($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function GetListFilterAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
		$codes = StationFactory::GetListFilter($columnName,$_GET['term']);
	...
	public static function GetListFKAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
			$foreignKey = end(StationFactory::GetForeignKeyParams($columnName));
			$codes = StationFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME,$foreignKey->REFERENCED_COLUMN_NAME,$_GET['term']);
			$data = new stdClass();
			foreach ($codes as $code){ 
				$obj = new stdClass(); 
	...
	public static function GetListDatatable() {
		...
		$aColumns = array('`oid`','`region_id`','`code`','`sequence_number`','`altitude`','`longitude`','`latitude`','`note`','`nickname`','DATE_FORMAT(`registration_date`,"%d/%m/%Y") as `registration_date`','`active`','`id`');
		$aColumnsName = array('oid','region_id','code','sequence_number','altitude','longitude','latitude','note','nickname','registration_date','active','id');
		$sIndexColumn = 'id';
		$sTable = 'pr_station';
		$gaSql['link'] = $db_conn;;
		/*         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
			...
			for ($i = 0; $i < count($aColumnsName); $i++) {
				if ($aColumnsName[$i] == 'id') {
					$row[] = $id;
                                        ...
                                        $Region = RegionLogic::Get($aRow[$aColumnsName[$i]]);
					$row[] = stripslashes("$Region->name");
