...
class ConfigParametersApiLogic
{
	...
	public static function Update($request){
		...
		try {
			...
			$ob = ConfigParametersLogic::Get($ob->id);

			CoreLogic::getFromArray($ob, $request->get("data"));

			CoreLogic::beginTransaction();
			$res = ConfigParametersLogic::Update($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function Erase($request) {
		try {
			...
			CoreLogic::beginTransaction();
			$res = ConfigParametersLogic::Erase($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function Delete($request) {
		try {
			...
			CoreLogic::beginTransaction();
			$res = ConfigParametersLogic::Delete($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function Get($id) {
		try {
			$res = false;
			$Person = CoreLogic::VerifyPerson();
			$ob = ConfigParametersLogic::Get($id);
			$res = true;
		} catch (ApiException $a) {
			return CoreLogic::GenerateErrorResponse($a->message);
		}
	...
	public static function GetList() {
		try {
			$Person = CoreLogic::VerifyPerson();
			$ob = ConfigParametersLogic::GetList();
			$res = true;
		} catch (ApiException $a) {
			return CoreLogic::GenerateErrorResponse($a->message);
		}
	...
	public static function GetListFilterAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
		$codes = ConfigParametersFactory::GetListFilter($columnName,$_GET['term']);
	...
	public static function GetListFKAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
			$foreignKey = end(ConfigParametersFactory::GetForeignKeyParams($columnName));
			$codes = ConfigParametersFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME,$foreignKey->REFERENCED_COLUMN_NAME,$_GET['term']);
			$data = new stdClass();
			foreach ($codes as $code){ 
				$obj = new stdClass(); 
	...
	public static function GetListDatatable() {
		...
		$aColumns = array('`oid`','`general_quiet`','`general_on_error`','`general_except`','`image_report_photo`','`daily_report_astro`','`daily_histo`','`monthly_report_astro`','`monthly_histo`','`event_fill_frames`','`event_recenter`','`event_box_bolide`','`event_model_psf`','`event_model_bar`','`event_report`','`event_image`','`event_video`','`id`');
		$aColumnsName = array('oid','general_quiet','general_on_error','general_except','image_report_photo','daily_report_astro','daily_histo','monthly_report_astro','monthly_histo','event_fill_frames','event_recenter','event_box_bolide','event_model_psf','event_model_bar','event_report','event_image','event_video','id');
		$sIndexColumn = 'id';
		$sTable = 'pr_config_parameters';
		$gaSql['link'] = $db_conn;;
		/*         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
