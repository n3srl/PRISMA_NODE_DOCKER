...
class RegionApiLogic
{
	...
	public static function Update($request){
		...
		try {
			...
			$ob = RegionLogic::Get($ob->id);

			CoreLogic::getFromArray($ob, $request->get("data"));

			CoreLogic::beginTransaction();
			$res = RegionLogic::Update($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function GetListFilterAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
		$codes = RegionFactory::GetListFilter($columnName,$_GET['term']);
	...
	public static function GetListFKAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
			$foreignKey = end(RegionFactory::GetForeignKeyParams($columnName));
			$codes = RegionFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME,$foreignKey->REFERENCED_COLUMN_NAME,$_GET['term']);
			$data = new stdClass();
			foreach ($codes as $code){ 
				$obj = new stdClass(); 
	...
	public static function GetListDatatable() {
		...
		$aColumns = array('`oid`','`state`','`code`','`name`','`id`');
		$aColumnsName = array('oid','state','code','name','id');
		$sIndexColumn = 'id';
		$sTable = 'pr_region';
		$gaSql['link'] = $db_conn;;
		/*         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
