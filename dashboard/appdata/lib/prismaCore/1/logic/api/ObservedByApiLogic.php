...
class ObservedByApiLogic
{
	...
	public static function Update($request){
		...
		try {
			...
			$ob = ObservedByLogic::Get($ob->id);

			CoreLogic::getFromArray($ob, $request->get("data"));

			CoreLogic::beginTransaction();
			$res = ObservedByLogic::Update($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function GetListFilterAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
		$codes = ObservedByFactory::GetListFilter($columnName,$_GET['term']);
	...
	public static function GetListFKAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
			$foreignKey = end(ObservedByFactory::GetForeignKeyParams($columnName));
			$codes = ObservedByFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME,$foreignKey->REFERENCED_COLUMN_NAME,$_GET['term']);
			$data = new stdClass();
			foreach ($codes as $code){ 
				$obj = new stdClass(); 
	...
	public static function GetListDatatable() {
		...
		$aColumns = array('`event_id`','`detection_id`','`id`');
		$aColumnsName = array('event_id','detection_id','id');
		$sIndexColumn = 'id';
		$sTable = 'pr_observed_by';
		$gaSql['link'] = $db_conn;;
		/*         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
