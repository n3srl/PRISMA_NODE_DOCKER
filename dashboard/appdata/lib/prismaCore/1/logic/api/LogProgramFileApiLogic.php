...
class LogProgramFileApiLogic
{
	...
	public static function Update($request){
		...
		try {
			...
			$ob = LogProgramFileLogic::Get($ob->id);

			CoreLogic::getFromArray($ob, $request->get("data"));

			CoreLogic::beginTransaction();
			$res = LogProgramFileLogic::Update($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function Erase($request) {
		try {
			...
			CoreLogic::beginTransaction();
			$res = LogProgramFileLogic::Erase($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function Delete($request) {
		try {
			...
			CoreLogic::beginTransaction();
			$res = LogProgramFileLogic::Delete($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function GetList() {
		try {
			$Person = CoreLogic::VerifyPerson();
			$ob = LogProgramFileLogic::GetList();
			$res = true;
		} catch (ApiException $a) {
			return CoreLogic::GenerateErrorResponse($a->message);
		}
	...
	public static function GetListFilterAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
		$codes = LogProgramFileFactory::GetListFilter($columnName,$_GET['term']);
	...
	public static function GetListFKAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
			$foreignKey = end(LogProgramFileFactory::GetForeignKeyParams($columnName));
			$codes = LogProgramFileFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME,$foreignKey->REFERENCED_COLUMN_NAME,$_GET['term']);
			$data = new stdClass();
			foreach ($codes as $code){ 
				$obj = new stdClass(); 
	...
	public static function GetListDatatable() {
		...
		$aColumns = array('`oid`','DATE_FORMAT(`timestamp`,"%d/%m/%Y %H:%i:%s UT") as `timestamp`','`type`','`level`','`text`','`id`,timestamp as timestamp_o');
		$aColumnsName = array('oid','timestamp','type','level','text','id');
		$aColumnsOrder = array('oid','timestamp_o','type','level','text','id');
		$sIndexColumn = 'id';
		$sTable = 'pr_log_program_file';
		$gaSql['link'] = $db_conn;;
		/*         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
