...
class EmailAlertApiLogic
{
	...
	public static function Update($request){
		...
		try {
			...
			$ob = EmailAlertLogic::Get($ob->id);

			CoreLogic::getFromArray($ob, $request->get("data"));

			CoreLogic::beginTransaction();
			$res = EmailAlertLogic::Update($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function GetListFilterAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
		$codes = EmailAlertFactory::GetListFilter($columnName,$_GET['term']);
	...
	public static function GetListFKAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
			$foreignKey = end(EmailAlertFactory::GetForeignKeyParams($columnName));
			$codes = EmailAlertFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME,$foreignKey->REFERENCED_COLUMN_NAME,$_GET['term']);
			$data = new stdClass();
			foreach ($codes as $code){ 
				$obj = new stdClass(); 
	...
	public static function GetListDatatable() {
		...
		$aColumns = array('`event_id`','`core_person_id`','`id`');
		$aColumnsName = array('event_id','core_person_id','id');
		$sIndexColumn = 'id';
		$sTable = 'pr_email_alert';
		$gaSql['link'] = $db_conn;;
		/*         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
