...
class EventExecutionHistoryApiLogic
{
	...
	public static function Update($request){
		...
		try {
			...
			$ob = EventExecutionHistoryLogic::Get($ob->id);

			CoreLogic::getFromArray($ob, $request->get("data"));

			CoreLogic::beginTransaction();
			$res = EventExecutionHistoryLogic::Update($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function Erase($request) {
		try {
			...
			CoreLogic::beginTransaction();
			$res = EventExecutionHistoryLogic::Erase($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function Delete($request) {
		try {
			...
			CoreLogic::beginTransaction();
			$res = EventExecutionHistoryLogic::Delete($ob);
			CoreLogic::commitTransaction();
		} catch (ApiException $a) {
			CoreLogic::rollbackTransaction();
			return CoreLogic::GenerateErrorResponse($a->message);
	...
	public static function Get($id) {
		try {
			$res = false;
			$Person = CoreLogic::VerifyPerson();
			$ob = EventExecutionHistoryLogic::Get($id);
			$res = true;
		} catch (ApiException $a) {
			return CoreLogic::GenerateErrorResponse($a->message);
		}
	...
	public static function GetList() {
		try {
			$Person = CoreLogic::VerifyPerson();
			$ob = EventExecutionHistoryLogic::GetList();
			$res = true;
		} catch (ApiException $a) {
			return CoreLogic::GenerateErrorResponse($a->message);
		}
	...
	public static function GetListFilterAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
		$codes = EventExecutionHistoryFactory::GetListFilter($columnName,$_GET['term']);
	...
	public static function GetListFKAjax($columnName) {
		try {
			...
			$results = array();
			$data = new stdClass();
			$foreignKey = end(EventExecutionHistoryFactory::GetForeignKeyParams($columnName));
			$codes = EventExecutionHistoryFactory::GetListFK($foreignKey->REFERENCED_TABLE_NAME,$foreignKey->REFERENCED_COLUMN_NAME,$_GET['term']);
			$data = new stdClass();
			foreach ($codes as $code){ 
				$obj = new stdClass(); 
	...
	public static function GetListDatatable() {
		...
		$aColumns = array('`oid`','`event_id`','DATE_FORMAT(`execution_datetime`,"%d/%m/%Y") as `execution_datetime`','`config_parameters`','`stdout`','`stderr`','`id`');
		$aColumnsName = array('oid','event_id','execution_datetime','config_parameters','stdout','stderr','id');
		$sIndexColumn = 'id';
		$sTable = 'pr_event_execution_history';
		$gaSql['link'] = $db_conn;;
		/*         * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
