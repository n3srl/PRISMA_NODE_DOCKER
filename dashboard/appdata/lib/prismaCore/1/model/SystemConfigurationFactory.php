 ...
 * @author: N3 S.r.l.
 */
class SystemConfigurationFactory extends SystemConfigurationFactoryBase 
{
	public static function CheckData($object, $clean = true){
		$errors = array();
		$parse_error = false;
		if ($object->parameter_name == '') {
			$errors[] = _('parameter_name obbligatorio');
			$parse_error = true;
		}
		if ($object->parameter_value == '') {
			$errors[] = _('parameter_value obbligatorio');
			$parse_error = true;
		}
