        ...
        case "$2" in
            "--list")
                sudo docker run --rm --network host \
                    ...
                    --name ft-temp n3srl/freeture -l
                ;;
            "--check")
                sudo docker run --rm --network host \
                    -v freeture-conf:/usr/local/share/freeture/ \
                    --name ft-temp n3srl/freeture -m 1
                ;;
            "--config")
                cat $FT_CONF
                ;;
        ...
        case "$2" in
            "--switch")
                ...
                then
                    ...
                    sudo docker exec -it prisma-vpn ping -c 5 -q $3 ; error=$?
                    if [ ${error} -eq 0 ]
                    then
                        echo "VPN: connection test passed"
                    else
                        echo "VPN: connection test failed"
                        if [ -f "${VPN_CONF}/${OLD_PROFILE}" ]
                        then
                    ...
                    #restart also the containers that depends on the VPN
                    echo "VPN: restarting SSH container...your SSH session will be interrupted."
                    sudo docker container restart -t 60 prisma-orma
